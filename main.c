...
HAL_TIM_Base_Start_IT(&htim16); //start timer in interrupt mode
  ...
  // TODO: Write patterns to the EEPROM using SPI
for (int i=0;i<6;i=i+1)
{
	write_to_address(i,patterns[i]); //loop through array to write each pattern to eeeprom
}
...
while (1) {
	...
	if (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_0) == 0) {
		...
		if (currentPeriod == shortPeriod) { // if half second change to one second
			currentPeriod = longPeriod;
		} else if(currentPeriod == longPeriod) { // if one
...
void TIM16_IRQHandler(void)
{
	...
	if (index>5){ // reset and restart if looped through all code
		index=0;
	};
    if(read_from_address(index)==patterns[index]){
		GPIOB->ODR = read_from_address(index);
		index++;
}
else{
	GPIOB->ODR = 1;
